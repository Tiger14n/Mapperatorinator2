...
from transformers import LogitsProcessorList, LogitsProcessor, EncoderDecoderCache, StaticCache, \
    ClassifierFreeGuidanceLogitsProcessor, Cache, DynamicCache, TemperatureLogitsWarper, TopPLogitsWarper, TopKLogitsWarper
...
class Processor(object):
    ...
    def generate(
        ...
        if generation_config.mapper_id != None:
            mapper_idx = torch.tensor([self.tokenizer.get_mapper_idx(generation_config.mapper_id)], dtype=torch.long, device=self.device)
        else:
            print("WARNING: Mapper ID not provided. Selecting a random mapper for target style.")
            mapper_idx = torch.randint(0, self.tokenizer.num_mapper_classes, (1,), dtype=torch.long, device=self.device)

        if generation_config.difficulty == None:
            print("WARNING: Difficulty not provided. Selecting 5.0 for difficulty.")
            generation_config.difficulty = 5.0
        difficulty = torch.tensor([generation_config.difficulty], dtype=torch.float32, device=self.device)
    ...
    def generate_sequential(
        ...
        for i, context in enumerate(out_context):
            ...
            for sequence_index, (frames, frame_time, full_frames, full_time) in enumerate(iterator):
                ...
                song_length = sequences[3][-1].item() + self.miliseconds_per_sequence
                global_pos_start = full_time / song_length # Start of the current frame within the full song
