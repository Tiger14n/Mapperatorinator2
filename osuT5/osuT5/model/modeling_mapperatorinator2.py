...
from .configuration_mapperatorinator2 import MapperatorinatorConfig
from .configuration_nwhisper import NWhisperConfig
from .modeling_nwhisper import NWhisperForConditionalGeneration
from ..model.spectrogram import MelSpectrogram2
...
class Mapperatorinator(PreTrainedModel):
    ...
    def __init__(self, config: MapperatorinatorConfig):
        ...
        self.spectrogram = MelSpectrogram2(
            config.sample_rate, config.n_fft,
            config.n_mels, config.hop_length
        )
    ...
    def forward(
            ...
            global_pos: Optional[torch.Tensor] = None,
        ...
        if encoder_outputs is None and frames is not None:
            ...
            input_features = torch.swapaxes(frames, 1, 2)  # [N, M, L]
            
            # Get difficulty embedding 
            diff_embedding = self.difficulty_embedder(difficulty)  # [N, D_diff]
            diff_embedding_expanded = diff_embedding.unsqueeze(2).repeat(1, 1, input_features.shape[2])
            
            # Get global position embedding
            pos_embedding = self.song_pos_embedder(global_pos)  # [N, D_pos]
            pos_embedding_expanded = pos_embedding.unsqueeze(2).repeat(1, 1, input_features.shape[2])

            # Get mapper embedding
            mapper_embedding = self.mapper_embedder(mapper_idx)  # [N, D_mapper]
            mapper_embedding_expanded = mapper_embedding.unsqueeze(2).repeat(1, 1, input_features.shape[2])
            
            # Concatenate all embeddings along the channel dimension
    ...
    def get_encoder(self):
        return OsuTEncoder(
            ...
            self.spectrogram,
            self.mapper_embedder,
            self.song_pos_embedder,
            self.difficulty_embedder,
...
class OsuTEncoder(nn.Module):
    def __init__(
            ...
            spectrogram: MelSpectrogram2,
            mapper_embedder: MapperStyleEmbedder,
            song_position_embedder: SongPositionEmbedder,
        ...
        self.song_position_embedder = song_position_embedder
    ...
    def forward(
        ...
        # Get global position embedding
        pos_embedding = self.song_position_embedder(global_pos)  # [N, D_pos]
        pos_embedding_expanded = pos_embedding.unsqueeze(2).repeat(1, 1, input_features.shape[2])
